...
import { AppView, GoogleAdsSubView, ActionLog, RoadmapTask, InsightCardData } from './types';
...
const App: React.FC = () => {
  ...
  // Legacy Data State (Kept for Dashboard Cards only)
  const [dashboardAdsInsight, setDashboardAdsInsight] = useState<InsightCardData | null>(null);

  // Roadmap State
  const [roadmapTasks, setRoadmapTasks] = useState<RoadmapTask[]>([
    { id: 't1', label: 'Audit Negative Keywords', subLabel: 'Completed today', completed: true, month: 1, impact: 'High', category: 'Waste Reduction' },
    { id: 't2', label: 'Optimize Ad Schedule for Store Hours', subLabel: 'Scheduled for Week 2', completed: false, month: 1, impact: 'Medium', category: 'Optimization' },
    { id: 't3', label: 'Review Geo-Targeting Efficiency', subLabel: 'Scheduled for Week 3', completed: false, month: 1, impact: 'Medium', category: 'Optimization' },
    { id: 't4', label: 'Launch "Local Inventory" Ads', subLabel: 'Requires Merchant Center Link', completed: false, month: 2, impact: 'High', category: 'Expansion' },
    { id: 't5', label: 'Competitor Conquesting Campaign', subLabel: 'Targeting "Guitar Center" terms', completed: false, month: 2, impact: 'High', category: 'Growth' },
  ]);

  // Fetch Dashboard-Level Data (Fast, Summary)
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const report = await GoogleAdsService.getNegativeKeywordReport('client_123');
        setDashboardAdsInsight(report.insight);
      } catch (error) {
  ...
  // Merge static insights with fetched Google Ads insight for the Main Dashboard
  const combinedInsights = [
    ...(dashboardAdsInsight ? [dashboardAdsInsight] : []),
    ...STATIC_INSIGHTS.filter(i => i.type !== 'WASTE') 
  ];

  const handleNavigateToSuite = (insightId?: string) => {
    setCurrentView(AppView.GOOGLE_ADS);
    if (insightId === 'waste-001') {
      // Map legacy ID to new view
      setGoogleAdsSubView(GoogleAdsSubView.KEYWORD_INTEL);
    } else {
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case AppView.GOOGLE_ADS:
        return (
          // We pass empty/default props for the new structure as the suite now self-fetches detail data
          // dashboardAdsInsight is passed just in case we need legacy 'waste' card reference
          <GoogleAdsSuite 
            initialSubView={googleAdsSubView} 
            wasteInsight={dashboardAdsInsight!}
            resolvedInsightIds={resolvedInsightIds}
