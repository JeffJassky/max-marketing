...
const loadBlocks = async () => {
  ...
  try {
    const res = await fetch(`/api/dashboard/blocks?${params.toString()}`).then(r => r.json());
    blocksData.value = res;
  } catch (e) {
    console.error('Failed to load dashboard blocks', e);
  } finally {
    ...
    >
      ...
      >
        ...
        >
          ...
          <div class="text-4xl font-black text-slate-900 mb-2">
            {{ formatCurrency(scorecard.scorecard.spend.value) }}
          </div>
          <div
            class="flex items-center gap-1 text-xs"
            :class="scorecard.scorecard.spend.change <= 0 ? 'text-green-600' : 'text-slate-500'"
          >
            <span class="font-bold"
              >{{ scorecard.scorecard.spend.change.toFixed(1) }}%</span
            >
          ...
          <div
            class="absolute top-full left-4 right-4 mt-2 bg-slate-900 text-white text-xs p-3 rounded-xl opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none z-50"
          >
            The total amount you've invested in advertising across all platforms
            (like Meta and Google) during this timeframe.
          </div>
        </div>
        ...
        >
          ...
          <div class="text-4xl font-black text-slate-900 mb-2">
            {{ formatCurrency(scorecard.scorecard.revenue.value) }}
          </div>
          <div
            class="flex items-center gap-1 text-xs"
            :class="scorecard.scorecard.revenue.change >= 0 ? 'text-green-600' : 'text-red-600'"
          >
            <component
              :is="scorecard.scorecard.revenue.change >= 0 ? ArrowUpRight : ArrowDownRight"
              size="14"
            />
            <span class="font-bold"
              >{{ Math.abs(scorecard.scorecard.revenue.change).toFixed(1)
          ...
          <div
            class="absolute top-full left-4 right-4 mt-2 bg-slate-900 text-white text-xs p-3 rounded-xl opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none z-50"
          >
            Your total gross sales recorded in Shopify. This includes all
            orders, whether they came from an ad, an email, or someone coming to
            your site directly.
          </div>
        </div>
        ...
        >
          <div class="flex justify-between items-start mb-4">
            <span
              class="text-slate-500 text-sm font-medium uppercase tracking-wider"
              >Acquisition Health</span
            >
            <Sparkles class="w-5 h-5 text-amber-500" />
          </div>
          <div class="text-4xl font-black text-slate-900 mb-2">
            {{ scorecard.scorecard.acquisition.value.toFixed(0) }}%
          </div>
          <div
            class="flex items-center gap-1 text-xs"
            :class="scorecard.scorecard.acquisition.change >= 0 ? 'text-green-600' : 'text-slate-500'"
          >
            <component
              :is="scorecard.scorecard.acquisition.change >= 0 ? ArrowUpRight : ArrowDownRight"
              size="14"
            />
            <span class="font-bold"
      ...
      <div
        ...
        v-if="scorecard && scorecard.scorecard.tcac"
      >
        <!-- True CAC -->
        <div
          class="bg-white p-8 rounded-[2.5rem] border border-stone-100 shadow-sm group/tooltip relative"
        >
          <div class="flex justify-between items-start mb-6">
            <div>
              <h3 class="text-xl font-bold text-slate-800">True CAC</h3>
              <p
                class="text-xs text-slate-400 mt-1 uppercase font-bold tracking-tighter"
              >
          ...
          <div
            class="absolute top-full left-4 right-4 mt-2 bg-slate-900 text-white text-xs p-3 rounded-xl opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none z-50"
          >
            The 'Attribution Reality Check.' This is exactly what you paid in
            ads to get one new customer into your store, using your actual
            customer data rather than platform guesses.
          </div>
          ...
          <div class="flex items-end gap-4 mb-8">
            ...
            <div class="pb-1">
              <div
                ...
                :class="scorecard.scorecard.tcac.change <= 0 ? 'text-green-600' : 'text-red-600'"
              >
                <component
                  :is="scorecard.scorecard.tcac.change <= 0 ? ArrowDownRight : ArrowUpRight"
                  size="14"
                />
          ...
          <div class="space-y-4">
            ...
            >
              ...
              <div
                class="text-sm font-black text-slate-400 line-through decoration-red-400/50"
              >
        ...
        >
          <div class="flex justify-between items-start mb-6">
            ...
            <div
              class="w-12 h-12 bg-amber-50 rounded-2xl flex items-center justify-center text-amber-600"
            >
              <Zap :size="24" />
            </div>
          </div>
          ...
          <div
            class="absolute top-full left-4 right-4 mt-2 bg-slate-900 text-white text-xs p-3 rounded-xl opacity-0 group-hover/tooltip:opacity-100 transition-opacity pointer-events-none z-50"
          >
            This highlights the difference between what platforms like Meta
            claim they are doing versus the actual profit you see in your bank
            account. It helps identify if platforms are 'over-claiming' credit
            for the same sale.
          </div>
          ...
          <div class="grid grid-cols-2 gap-8 mb-8">
            ...
            <div>
              ...
              <div
                class="text-3xl font-black text-slate-400 line-through decoration-red-400/50"
              >
          ...
          <div class="relative pt-4">
            ...
            <div class="text-center mb-4">
              <div
                ...
                :class="(scorecard.scorecard.tcac.efficiencyGap ?? (scorecard.scorecard.mer.value - scorecard.scorecard.tcac.platformRoas)) >= 0 ? 'text-green-600' : 'text-red-500'"
              >
                {{ (scorecard.scorecard.tcac.efficiencyGap ?? (scorecard.scorecard.mer.value - scorecard.scorecard.tcac.platformRoas)) >= 0 ? '+' : ''
      ...
      >
        <div class="flex justify-between items-center mb-8">
          ...
          >
            A bird's-eye view of where your money is going. This helps you
            ensure you aren't over-investing in one area (like Social) while
            neglecting another (like Search).
          </div>
        </div>

        <div v-if="spendDataLoading" class="py-20 flex justify-center">
          <RefreshCw class="animate-spin text-indigo-200" :size="40" />
        </div>
        <div
          v-else-if="grandTotal === 0"
          class="py-20 text-center text-slate-300 italic"
        >
          Connect your ad accounts to see spend breakdown.
        </div>

        <template v-else>
          <div
            class="w-full flex h-12 mb-12 rounded-2xl overflow-hidden shadow-inner border border-slate-100"
          >
            <div
              v-for="s in googleSegments"
              :key="s.id"
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
            <div>
              <div
                class="flex items-center justify-between mb-4 border-b border-slate-50 pb-2"
              >
                <span
                  class="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2"
                  ><Target size="14" class="text-blue-500" /> Google Ads</span
                >
              </div>
              <div class="space-y-4">
                <div
            ...
            <div>
              ...
              >
                <span
                  class="text-xs font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2"
                  ><LayoutGrid size="14" class="text-indigo-500" /> Meta
                  Ads</span
                >
              </div>
              <div class="space-y-4">
