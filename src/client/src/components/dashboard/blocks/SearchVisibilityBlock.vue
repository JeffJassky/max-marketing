...
<template>
  <div class="bg-white p-7 rounded-2xl shadow-[0_1px_3px_rgba(0,0,0,0.06),0_4px_12px_rgba(0,0,0,0.04)] group/block relative">
    <div class="flex items-center justify-between mb-5">
      <span class="text-xs font-bold tracking-[0.1em] uppercase text-gray-500">Search Visibility</span>
    </div>
    ...
    <div class="mb-1">
      ...
      <div class="text-xs text-gray-400 mt-1 mb-1.5">Total times your site appeared in Google search results</div>
    ...
    <div class="text-[13px] font-bold text-slate-900 mb-2">Branded vs. Non-Branded Searches</div>

    <div class="h-[22px] rounded-md overflow-hidden flex mb-1.5">
      <div
        :style="{ width: `${brandedPct}%` }"
        class="h-full bg-indigo-500 flex items-center justify-center text-[11px] font-semibold text-white"
      >{{ brandedPct }}%</div>
      <div
        :style="{ width: `${nonBrandedPct}%` }"
        class="h-full bg-violet-300 flex items-center justify-center text-[11px] font-semibold text-slate-900"
      >{{ nonBrandedPct }}%</div>
    </div>
    ...
    <div class="mt-1">
      <div class="flex justify-between items-baseline mb-1">
        <span class="text-[13px] font-bold text-slate-900">Impressions to Clicks</span>
