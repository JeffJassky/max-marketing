...
const defineBronzeImport = <E extends WindsorEndpoint>(preset: WindsorImportQuery<E>) => preset
...
export const windsorImportQueries = {
	...
	googleAdsKeywordQuality: defineBronzeImport({
		...
		request: {
			...
			dimensions: ['campaign', 'ad_group', 'keyword_text', 'keyword_match_type'],
			params: {
				date_preset: 'last_30d'
			}
		},
	...
	googleAdsSearchTerms: defineBronzeImport({
		...
		description: 'Search term level performance mapped to matched keywords for query mining.',
		request: {
			endpoint: 'googleAdsSearchTerms',
			metrics: ['spend', 'impressions', 'clicks', 'conversions', 'ctr', 'average_cpc'],
			dimensions: [
				'date',
				'campaign',
				'ad_group',
	...
	googleAdsKeywordDeviceNetwork: defineBronzeImport({
		...
		request: {
			...
			metrics: ['spend', 'impressions', 'clicks', 'conversions', 'conversion_rate'],
			dimensions: [
				'date',
				'campaign',
				'ad_group',
	...
	googleAdsKeywordTrends: defineBronzeImport({
		...
		description: 'Daily keyword time-series for monitoring directional shifts and seasonality.',
		request: {
	...
	googleAdsKeywordCpaRanking: defineBronzeImport({
		...
		request: {
			...
			metrics: ['cost_per_conversion', 'conversions', 'spend', 'impressions', 'clicks'],
			dimensions: ['campaign', 'ad_group', 'keyword_text', 'keyword_match_type'],
			params: {
				date_preset: 'last_30d',
