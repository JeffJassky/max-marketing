...
import { wastedSpendKeyword } from '../entities/keyword-daily/signals/wasted-spend-keyword.signal';
...
describe('wastedSpendKeyword pipeline (live)', () => {
	...
	it('executes the wastedSpendKeyword signal against silver and writes gold_signals data', async () => {
		...
		await signalExecutor.run(wastedSpendKeyword);
		...
		const [rows] = await bq.query({
			...
			query: `
          SELECT *
          FROM \`${wastedSpendKeyword.dataset}.${wastedSpendKeyword.tableName}\`
